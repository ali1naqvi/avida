...
#include "config_overrides.h"
  ...
  CONFIG_ADD_VAR(GENERATION_SELECTION, int, 0, "Enable explicit generation-level energy selection.\n0 = OFF (default, prior behavior)\n1 = ON: organisms execute until the stop instruction or timeout, then selected genomes are copied into the next locked generation.");
  CONFIG_ADD_VAR(EPISODE_LENGTH, int, 100, "Maximum updates in each episode (generation-selection evaluation window).\nAn episode ends early when all organisms execute stop; otherwise it ends after this many updates using current energy for unfinished organisms.");
  CONFIG_ADD_VAR(NUM_EPISODES, int, 1, "Number of episodes evaluated per generation when GENERATION_SELECTION is enabled.\nEach generation runs this many episodes; at the start of every episode each individual is reset (energy/CPU state) and placed at a fresh random world position per OFFSPRING_WORLD_POS.\nAn individual's selection energy is the MEAN of its per-episode terminal energies.\nAfter the final episode, selected genomes are copied into the next locked generation.\n1 = single episode per generation (default; equivalent to prior behavior).");
  CONFIG_ADD_VAR(GENERATION_SELECTION_TOURNAMENT_K, int, 2, "Tournament size for generation-level terminal-energy selection.\nEach selected genome is the stopped/timeout organism with the highest terminal energy sampled from k candidates. Values <= 0 are treated as 1.");
  CONFIG_ADD_VAR(GENERATION_SELECTION_ELITES, int, 0, "Number of highest-terminal-energy organisms copied unchanged into the next locked generation before tournament selection.\n0 = no elites/prior behavior.");
  CONFIG_ADD_VAR(GENERATION_SELECTION_SHADOW_RUN, bool, 0, "Use neutral parent selection for explicit generation selection.\n0 = normal energy-based elitism and tournament winners.\n1 = select the first uniformly random member of each tournament sample; elites are disabled. The draw uses Avida's seeded RNG, so runs are reproducible for a fixed RANDOM_SEED.");
