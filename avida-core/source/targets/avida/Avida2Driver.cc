...
void Avida2Driver::Run()
{
  ...
  while (!m_done) {
    ...
    if (m_world->GetVerbosity() > VERBOSE_SILENT) {
      ...
      // Energy stats: live-population ave/max use GetAveFitness/GetMaxFitness; death stats use
      // stored energy at KillOrganism; BestEnergy is the all-time highest stored energy seen.
      cout << "Energy: " << setw(9) << setprecision(7) << stats.GetAveFitness() << "  ";
      cout << "MaxEnergy: " << setw(8) << setprecision(7) << stats.GetMaxFitness() << "  ";
      const cPopulation::LifetimeFitnessChampion& lifetime_champ = population.GetLifetimeFitnessChampion();
      cout << "BestEnergy: " << setw(6) << setprecision(7) << (lifetime_champ.valid ? lifetime_champ.stored_energy : 0.0) << "  ";
